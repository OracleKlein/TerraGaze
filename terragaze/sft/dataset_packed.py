...
from teonext.utils.constants import IMG_CONTEXT_TOKEN, IMG_END_TOKEN, IMG_START_TOKEN
...
class PackedDataset(IterableDataset):
    def __init__(
        ...
        #| Actually, unk_token_id is used to represent those tokens could not be tokenized. Not onhold token id.
        #| Here, ensure img_start_token_id, img_token_id, img_end_token_id are not converted to unk_token_id because
        #| of failed insertion of special tokens into the tokenizer.
        ...
        self.dataset_type = [d.dataset_type for d in self.datasets] #| set to 'pair' in LazySupervisedDataset
        ...
        #| _state_dict can be considered as a global variable for each worker to synchronize the states of each dataset
        self._state_dict = {
        ...
        #| In LazySupervisedDataset, max_num_images is set to 1.
        #| In config sh, num_images_expected is set to 96 referring to the number of image patches in packed sequence.
    ...
    def next_data(self, current_dataset_idx):
        while True:
            try:
                ...
                if self.replacement: #| If allowed, restart the exhausted dataset by reinitializing its iterator.
                    ...
                    try:
                        ...
                        current_sample = next(self.dataset_iter_list[current_dataset_idx])
                        break
                    except: #| Remove selected dataset and evenly sample from the remaining datasets.
                ...
                else: #| Remove selected dataset and evenly sample from the remaining datasets.
        ...
        #| add type_ids to current_sample, such as pixel_values, input_ids, labels, attention_mask, position_ids, etc.
    ...
    #| find the very first buffer could take the new sample according to the number of images and tokens, if not found, return None
    ...
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id, img_end_token_id):
        ...
        while buffer['input_ids'].size(0) > max_tokens:
            #| find the positions of image start token and image end token
            ...
            if _image_is_splitted(buffer['input_ids'], max_tokens): #| whether max_tokens cut through an image
                #| bisect_left finds the first position of number >= max_tokens
                #| right/bisect finds the first position of number > max_tokens
                ...
                #| the very first position of image start token > max_tokens
                cut_img_idx = bisect.bisect(img_start_idx_list, max_tokens)
                if cut_img_idx < len(img_start_idx_list): #| exist at least one image start token on the right side of max_tokens
                    cut_right_idx = img_start_idx_list[cut_img_idx]
                    cut_right_img_idx = cut_img_idx
                else: #| all image tokens are on the left side of max_tokens
                    cut_right_idx = None
    ...
    def update_buffer_list(self, buffer_list, buffer_max_len_list, buffer):
        ...
        #| split_buffer split buffer into multiple sub-buffers, this may break the original conversation up, so allow_overflow
        #| should be set to false to avoid this and no single conversation is supposed to exceed max_packed_tokens
        ...
        #| splitted_buffer contains left and right parts of the original buffer.
        for each_buffer in splitted_buffer:
            ...
            #| insert splited buffer into buffer_list in descending order of number of images
    ...
    #| sample: input_ids, attention_mask, position_ids, labels, image_flags, pixel_values
    #| buffer: sample + data_index, type_ids, worker_state_key, worker_state_dict, custom_infos
    def __iter__(self):
        iter_idx = 0
        buffer_list = []
        buffer_max_len_list = []
        ...
        #| get_worker_info() get cpu worker info from current GPU process
        worker_id = 0 if get_worker_info() is None else get_worker_info().id
        num_workers = 1 if get_worker_info() is None else get_worker_info().num_workers #| how many cpu workers for each process

        worker_id = num_workers * self.data_rank + worker_id #| change to global worker id
        num_workers = num_workers * self.data_world_size

        rng = np.random.default_rng(seed=worker_id) #| same between different gpu processes

        # reset states of each dataset
        self.worker_id = worker_id #| current cpu worker id
        ...
        #| buffer's field is same as the sample returned by dataset with additional fields
        ...
        while True:
            ...
            try:
                ...
                while len(buffer_list) > 0:
                    if self.strict_mode: #| defalt to false
                ...
                return
            #| find the first buffer that can take the current sample, if not found, return None
            buffer = self.find_buffer(buffer_list, current_sample)
            #| If found, merge the current sample into the buffer, otherwise, the current sample will be directly treated as a new buffer.
            ...
            while len(buffer_max_len_list) > 0:
                ...
                #| strict_mode is set to false by default
            ...
            #| buffer_list is sorted by number of images in descending order.
    ...
    def get_cu_seqlens_and_indexes(
        ...
        for i in range(start, end):
            ...
            indexes.extend(list(range(num_tokens))) #| [0...999, 0...1999, 0...1095]
            cu_seqlens.append(cu_seqlens[-1] + num_tokens) #| [0,1000,3000,4096] find self-contained conversation
            assert num_tokens > 0

            curr_data_index = data_index[cu_seqlens[-2]:cu_seqlens[-2]+num_tokens]
            assert (curr_data_index == i).all(), data_index

            #| compute loss_weight according to non_ignore_loss token number
...
def packed_collate_fn(
    ...
    len2weight: callable,
    max_item_length: int,
    micro_num: int = 1, #| batch_size for single GPU
    pad_id: int = 0,
):
    ...
    while len(features) < micro_num: #| create dummy samples
    ...
    #| if strict_mode enabled, set to max_packed_len, else set to dynamic padding
    ...
    for feat_idx, feat in enumerate(features):
        ...
        #| add loss_weight to feature according to num_effective_tokens
        feat['loss_weight'] = curr_loss_weight

        if feat_idx < num_features: #| real samples
            num_samples += len(curr_cu_seqlens) - 1 #| total number of real sample in one batch

        if curr_cu_seqlens[-1] < max_item_length: #| padding to same length
            curr_cu_seqlens.append(max_item_length)
            curr_indexes.extend(list(range(max_item_length - curr_cu_seqlens[-2])))

        #| stack batch, commonly only one sample here
    ...
    #| call collator function to pad batches, actually
    batch = data_collator(features=features, max_item_length=max_item_length, pad_id=pad_id)
    #| batch : input_ids, labels, attention_mask, position_ids, loss_weight, pixel_values, image_flags, type_ids
    #| input_ids, labels, attention_mask, position_ids, loss_weight: [batch, seq]
    #| pixel_values : [num_images, channel, height, width] and image_flags : [num_images]
    ...
    batch['statistics'] = torch.tensor(
        [
            num_samples, #| real sample
            num_effective_tokens, #| real tokens
            num_padding_tokens, #| padding tokens
            batch['image_flags'].numel() - batch['image_flags'].sum().item(), #| padding images
        ],
        dtype=torch.long,
    )
    batch.pop('type_ids') #| used to represent source of datasets
    return batch
    #| batch : input_ids, labels, attention_mask, position_ids, loss_weight, cu_seqlens, pixel_values, image_flags, statistics
