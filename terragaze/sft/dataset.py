...
from teonext.utils.utils import dynamic_preprocess
from teonext.utils.utils import expand2square

from teonext.utils.s3_fileio import Client
...
#| wrapper function to simulate jpeg degradation
...
#| is_train corresponds to 'data_augment' field in dataset meta file
#| transform: RandomDegrade, Resize, ToTensor, Normalize
#| If dynamic patching is enabled, resize has happened in dynamic_preprocess, all patch will be the same size of
#| input_size or force_image_size in config file
def build_transform(is_train, input_size, pad2square=False, normalize_type='imagenet'):
    ...
    if is_train:  # use data augumentation
        transform = T.Compose([
            ...
            #| RandomChoice will choose one of the jpeg_degrade_functions randomly
            #| Randomly degrade the image quality, making training more robust
...
#| supports multi-image pretraining
def preprocess_pretrain(
        ...
        ds_name: str = None,
        num_image: int = 1 #| num_image comes from length of data_item['image']
) -> Dict:
    assert len(sources) == 1, 'process only the first conversations'
    conversations = sources[0]
    assert len(conversations) == 1 #| pretrain data should be single-turn conversation with 'from' == 'pretrain'
    ...
    if not text_only:
        ...
        for conversation in conversations:
            ...
            for i in range(image_cnt):
                ...
                conversation['value'] = conversation['value'].replace('<image>', image_tokens, 1) #| only replace the first <image> token
                current_image_idx += 1
...
def preprocess_internvl2_5(
    ...
    #| Exchange <image> token with image placeholder tokens
    if not text_only:
        new_conversations = []
        current_image_idx = 0
        for conversation in conversations:
    ...
    #| The main difference between InternVL and LLaVA is that conversation template evolves into ChatML which gets
    #| rid of the problem of space tokenization, in Fastchat or LLaVA, tokenization of individual turns and whole
...
def preprocess_fastchat_chatml(
    template_name,
    sources,
    tokenizer: transformers.PreTrainedTokenizer,
    num_image_token_list: list,
    ...
    batches, roles = [], []
    if system_prompt is not None:
        batches.append(conv_template.system_template.format(system_message=system_prompt) + conv_template.sep + "\n")
        roles.append("system")

    user_role, assistant_role = conv_template.roles
    for conversation in conversations:
        if conversation["from"] == "human":
            batches.append(f"{user_role}\n{conversation['value']}{conv_template.sep}\n")
            roles.append("human")
        elif conversation["from"] == "gpt":
            batches.append(f"{assistant_role}\n{conversation['value']}{conv_template.sep}\n")
            roles.append("gpt")
        else:
            raise NotImplementedError
    ...
    final_input_ids, final_targets = [], []
    ignore_ids = tokenizer(f"{assistant_role}\n", return_tensors="np").input_ids[0]
    ignore_len = ignore_ids.shape[0] - 1 if add_bos_token else ignore_ids.shape[0]
    for role, input_id in zip(roles, input_ids):
        final_input_ids.append(input_id)
